...
const randomGreeting =
  ...
  const totalCustomer = new Set(
    invoices.map(inv => inv.kepada || inv.customer)
  ).size;

  const paidCount    = invoices.filter(inv => getComputedStatus(inv) === "paid").length;
  const overdueCount = invoices.filter(inv => getComputedStatus(inv) === "overdue").length;
  const unpaidCount  = invoices.filter(inv => getComputedStatus(inv) === "unpaid").length;
  ...
  return (
    <div className="dashboard-container">
      ...
      <div className="dashboard-header-card">
        ...
        <p>
          {timeGreeting}, <strong>{user?.name || "Admin"}</strong> 👋
        </p>

        <span className="dashboard-motivation">
          {randomGreeting}
        </span>
      ...
      <div className="charts-wrapper">
        ...
        <div className="chart-card small">
          ...
          {/* 
            ← height eksplisit di wrapper dan minHeight di ResponsiveContainer
               agar donut muncul di mobile 
          */}
